...
def get_flags(*, parse_flags: bool = True, flags_dict: dict = {}) -> dict:
    ...
    parser.add_argument('-f', '--savefolder', type=str, default="", help="Absolute path of folder / file to save to")
...
def main():
    ...
    if args_value['savefolder'] != "":
        print('\033[96m' + f"Folder input detected, ignoring drive inputs" + '\033[0m')
    ...
    print('\033[93m' + 
              f"Backing up {args_value['filepath_to_backup']} to {f'drive {drive_letter}({volume_name})' if not args_value['savefolder'] else args_value['savefolder']} with flags --verbose={args_value["verbose"]} and --dryrun={args_value["dryrun"]}"
              + '\033[0m')
    
    if not args_value["dryrun"] and not args_value["yes"]: # Skips confirmation message
    ...
    if args_value['savefolder'] != "":
        backup_files_to_folder(args_value["savefolder"], args_value["relativepath"], args_value["filepath_to_backup"], args_value["verbose"], args_value["dryrun"])

    else:
        backup_files_to_drive(args_value["serial"], args_value["relativepath"], args_value["filepath_to_backup"], args_value["verbose"], args_value["dryrun"])
